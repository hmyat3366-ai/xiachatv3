...
export const ConversationListPanel: React.FC<ConversationListPanelProps> = ({
  ...
  return (
    <div className="w-full lg:w-[330px] xl:w-[360px] 2xl:w-[390px] shrink-0 border-r border-[#E8E8E5] bg-white flex flex-col h-full z-10 select-none">
      {/* ─────────────────────────────────────────────────────────────
          1. HEADER & SEARCH BAR
         ───────────────────────────────────────────────────────────── */}
      <div className="p-3 border-b border-[#E8E8E5] space-y-2.5 bg-[#FAF9F6]/80">
        {/* Row 1: Conversations Title + Live/Offline Badge on Left, Visitors Action on Right */}
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <h2 className="font-black text-base text-[#171717] tracking-tight shrink-0">Conversations</h2>
            <span className="px-2 py-0.5 rounded-full bg-[#FFF0E5] text-[#D96512] text-[11px] font-black shrink-0">
              {stats.total}
            </span>
            ...
            <span
              title={
                sseStatus === 'connected'
                  ...
                  : 'Offline — using background auto-sync'
              }
              className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold border shrink-0 ${
                sseStatus === 'connected'
            ...
            >
              {sseStatus === 'connected' ? (
                <><Wifi className="w-2.5 h-2.5 text-emerald-600 shrink-0" /> Live</>
              ) : sseStatus === 'connecting' ? (
                <><span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shrink-0" /> Syncing</>
              ) : (
                <><WifiOff className="w-2.5 h-2.5 text-gray-400 shrink-0" /> Offline</>
              )}
            </span>
          </div>

          {/* Visitors Drawer Trigger */}
          {onOpenLiveVisitors && (
            <button
              type="button"
              onClick={onOpenLiveVisitors}
              className="h-7 px-2.5 rounded-xl text-[11px] font-bold flex items-center gap-1.5 transition-all cursor-pointer bg-emerald-50 hover:bg-emerald-100 active:bg-emerald-200 text-emerald-800 border border-emerald-200 shadow-2xs shrink-0"
        ...
        {/* Row 2: Search Bar + Filter Button (Cleanly aligned side-by-side with breathing room) */}
        <div className="flex items-center gap-1.5">
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full pl-8 pr-7 py-1.5 rounded-xl text-xs font-medium bg-white border border-[#E8E8E5] text-[#171717] placeholder:text-gray-400 focus:outline-none focus:border-[#FF8A2A] focus:ring-2 focus:ring-[#FF8A2A]/15 transition-all"
            />
            {filters.search && (
              <button
          ...
          <button
            ...
            className={`h-7 px-2.5 rounded-xl text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer border shrink-0 ${
              activeFilterCount > 0
                ? 'bg-[#FFF0E5] border-[#FF8A2A] text-[#D96512] ring-2 ring-[#FF8A2A]/20'
                : 'bg-white border-[#E8E8E5] text-[#6B6B6B] hover:text-[#171717] hover:bg-[#FAF9F6]'
            }`}
